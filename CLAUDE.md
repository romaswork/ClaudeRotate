...
- `KeyRotator/RootView.swift` — UI: вкладка «Обзор» (`DashboardView`: карточка
  ...
  автостарт, язык интерфейса, секция «Данные»: экспорт настроек в файл
  (`NSSavePanel` → `store.exportData`), импорт (`NSOpenPanel` → `store.importData`)
  и сброс всех настроек с подтверждением через `.alert` → `store.resetAll`). Все
  строки UI — через `store.tr(...)`.
