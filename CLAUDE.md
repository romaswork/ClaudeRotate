...
- `KeyRotator/RootView.swift` — UI. Навигация — нативный `NavigationSplitView` с
  ...
  «Настройки» (секция «Целевые файлы» — две строки `targetFileRow` с тоггл-флагом
  включения и кнопкой «Выбрать…» через `NSOpenPanel`: Claude → `store.setTargetFile`,
  Codex → `store.setCodexFile`, общий пикер `pickJSON(near:)` пред-навигирует к
  `~/.claude/settings.json` / `~/.codex/auth.json`; смена тоггла сразу применяет
  текущий ключ; интервал — `TextField`+`Stepper` и segmented-пресеты
