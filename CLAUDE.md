...
- `KeyRotator/RootView.swift` — UI. Навигация — нативный `NavigationSplitView` с
  ...
  до 20 символов (`displayName`); справа идут две капсулы (стиль `tagChip`): сначала
  капсула ключа (`keySuffixChip`) с иконкой `key` и последними 3 символами API-ключа
  в формате `..Jj4` (`keySuffix`), затем левее правого края — капсула прокси
  (`proxyChip`) с иконкой `network`. Обе капсулы кликабельны (`Button(.plain)`) и
